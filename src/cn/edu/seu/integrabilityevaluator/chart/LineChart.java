...
public abstract class LineChart {
	...
	public void createChart() throws IOException {
		...
		/*
		 * renderer.setShapesVisible(true); renderer.setDrawOutlines(true);
		...
		/*
		 * // 创建文件输出流 File fos_jpg = new File("E://"+string+".jpg "); //
	...
	public void setDataSet(
			LinkedHashMap<String, HashMap<String, Double>> dataMap) {
		// DefaultCategoryDataset dataset = new DefaultCategoryDataset();
		Iterator<Entry<String, HashMap<String, Double>>> it = dataMap
				.entrySet().iterator();
		while (it.hasNext()) {
			Entry<String, HashMap<String, Double>> entry1 = it.next();

			for (Map.Entry<String, Double> entry2 : entry1.getValue()
					.entrySet()) {
				// System.out.println("key= " + entry2.getKey() + " and value= "
				// + entry2.getValue());
				dataset.addValue(entry2.getValue(), entry2.getKey(),
						entry1.getKey());
			}
		}
		...
		for (int i = 0; i < dataset.getRowCount(); i++) {
			...
			for (int j = 0; j < dataset.getColumnCount(); j++) {
				Number number = dataset.getValue(rowKey,
