 ...
 */
public class SeriesBugLineChart {
	...
	public static JFreeChart createChart(CategoryDataset categoryDataset) {
		...
		// 设置Y轴起始数据为数据集中最小值的0.8倍,先判断数据是否为空
	...
	public static CategoryDataset createDataset(String projName) {
		...
		// 查询所有halstead数据
		String sql = "select * from t_systemhalstead where projectName ='"
				+ projName + "'";
		ResultSet rs_hsd = queryClassMccData.queryInfo(sql);
		try {
			while (rs_hsd.next()) {
				String proj = rs_hsd.getString(2) + rs_hsd.getString(3);
				float bugs = rs_hsd.getFloat(10);
				dataset.addValue(bugs, series3, proj);
			}
