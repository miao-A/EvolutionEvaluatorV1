 ...
 */
public class SeriesEffortLineChart {
	...
	public static CategoryDataset createDataset(String projName) {
		...
		// 折线的含义或者说类型
		String series4 = "effort";
		DefaultCategoryDataset dataset = new DefaultCategoryDataset();
        //查询所有halstead数据
		String sql = "select * from t_systemhalstead where projectName ='"+projName+"'";
		ResultSet rs_hsd = queryClassMccData.queryInfo(sql);
		try {
			while (rs_hsd.next()) {
					String proj = rs_hsd.getString(2) + rs_hsd.getString(3);
					float effort = rs_hsd.getFloat(11);
					dataset.addValue(effort, series4, proj);
